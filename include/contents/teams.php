...
function show_members ($gid,$tpl) {
    ...
    $q = "SELECT b.uid, a.skype, a.avatar, a.status, a.name, c.name as posi, staat FROM prefix_groupusers b LEFT JOIN prefix_user a ON a.id = b.uid LEFT JOIN prefix_groupfuncs c ON b.fid = c.id WHERE b.gid = ".$gid." ORDER BY c.pos ASC, a.name ASC";
    $erg = db_query($q);
	...
	while($row = db_fetch_assoc($erg) ) {
            ...
            if(!empty($row['skype'])){
                $row['skype'] = '<a href="skype:'.$row['skype'].'?userinfo" title="'.$row['skype'].'" title="'.$row['skype'].'">'.$row['skype'].'</a>';
            } else {
                $row['skype'] = 'n/a';
            }
