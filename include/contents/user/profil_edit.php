...
if ( $_SESSION['authright'] <= -1 ) {
    ...
    if ( empty ($_POST['submit']) || !$csrfCheck ) {
        $design->header();
        $abf = 'SELECT email,wohnort,homepage,skype,steam,battlenet,avatar,status,staat,gebdatum,sig,opt_pm_popup,opt_pm,opt_mail,geschlecht,spezrank FROM `prefix_user` WHERE id = "'.$_SESSION['authid'].'"';
        $erg = db_query($abf);
