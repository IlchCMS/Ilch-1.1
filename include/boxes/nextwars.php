...
if ( @db_num_rows($erg) == 0 ) {
	...
	while ($row = @db_fetch_object($erg) ) {
		...
		echo '<div class="tdweight90 text-left">'.get_wargameimg($row->game).'<a class="box" href="index.php?wars-more-'.$row->id.'">'.$row->tag.'</a><br><span class="smalfont">'.$row->time.' Uhr</span></div>';
	}
}
echo '</div>';
?>