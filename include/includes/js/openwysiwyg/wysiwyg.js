...
// Images Directory
imagesDir = "include/includes/js/openwysiwyg/icons/";
imagesDir = "include/images/icons/";

// CSS Directory
cssDir = "include/includes/js/openwysiwyg/styles/";

// Popups Directory
popupsDir = "include/includes/js/openwysiwyg/popups/";
...
var buttonName = new Array();
  ...
  buttonName[22]  = "seperator";
	//buttonName[23]  = "inserttable";
  buttonName[24]  = "insertimage";
  buttonName[25]  = "createlink";
	buttonName[26]  = "seperator";
	buttonName[27]  = "icode";
	buttonName[28]  = "viewSource";
...
//Name              buttonID                 buttonTitle           buttonImage                            buttonImageRollover
  "bold":           ['Bold',                 'Bold',               imagesDir + 'button.bold.gif',               imagesDir + 'bold_on.gif'],
  "italic":         ['Italic',               'Italic',             imagesDir + 'button.italic.gif',            imagesDir + 'italics_on.gif'],
  "underline":      ['Underline',            'Underline',          imagesDir + 'button.underline.gif',          imagesDir + 'underline_on.gif'],
	"strikethrough":  ['Strikethrough',        'Strikethrough',      imagesDir + 'strikethrough.gif',      imagesDir + 'strikethrough_on.gif'],
	"seperator":      ['',                     '',                   imagesDir + 'separator.gif',          imagesDir + 'seperator.gif'],
	...
	"justifyleft":    ['Justifyleft',          'Justifyleft',        imagesDir + 'button.justifyleft.gif',       imagesDir + 'justify_left_on.gif'],
	"justifycenter":  ['Justifycenter',        'Justifycenter',      imagesDir + 'button.justifycenter.gif',     imagesDir + 'justify_center_on.gif'],
	"justifyright":   ['Justifyright',         'Justifyright',       imagesDir + 'button.justifyright.gif',      imagesDir + 'justify_right_on.gif'],
	"unorderedlist":  ['InsertUnorderedList',  'InsertUnorderedList',imagesDir + 'button.insertunorderedlist.gif',     imagesDir + 'list_unordered_on.gif'],
	"orderedlist":    ['InsertOrderedList',    'InsertOrderedList',  imagesDir + 'button.insertorderedlist.gif',       imagesDir + 'list_ordered_on.gif'],
	"outdent":        ['Outdent',              'Outdent',            imagesDir + 'button.indent_left.gif',        imagesDir + 'indent_left_on.gif'],
	"indent":         ['Indent',               'Indent',             imagesDir + 'button.indent_right.gif',       imagesDir + 'indent_right_on.gif'],
  ...
  "paste":          ['Paste',                'Paste',              imagesDir + 'paste.gif',              imagesDir + 'paste_on.gif'],
	"forecolor":      ['ForeColor',            'ForeColor',          imagesDir + 'button.forecolor.gif',          imagesDir + 'forecolor_on.gif'],
	"backcolor":      ['BackColor',            'BackColor',          imagesDir + 'button.backcolor.gif',          imagesDir + 'backcolor_on.gif'],
	"undo":           ['Undo',                 'Undo',               imagesDir + 'button.undo.gif',               imagesDir + 'undo_on.gif'],
	"redo":           ['Redo',                 'Redo',               imagesDir + 'button.redo.gif',               imagesDir + 'redo_on.gif'],
	"inserttable":    ['InsertTable',          'InsertTable',        imagesDir + 'insert_table.gif',       imagesDir + 'insert_table_on.gif'],
	"insertimage":    ['InsertImage',          'InsertImage',        imagesDir + 'button.image.gif',     imagesDir + 'insert_picture_on.gif'],
	"createlink":     ['CreateLink',           'CreateLink',         imagesDir + 'button.link.gif',   imagesDir + 'insert_hyperlink_on.gif'],
	"icode":          ['iCode',                'Code',               imagesDir + 'button.code.gif',               imagesDir + 'bold_on.gif'],
	"viewSource":     ['ViewSource',           'ViewSource',         imagesDir + 'button.view_source.gif',        imagesDir + 'view_source_on.gif'],
	"viewText":       ['ViewText',             'ViewText',           imagesDir + 'button.view_text.gif',          imagesDir + 'view_text_on.gif'],
	...
	"selectsize":     ['SelectSize',           'SelectSize',         imagesDir + 'button.size.gif',        imagesDir + 'select_size_on.gif']
};
...
function generate_wysiwyg(textareaID) {
  ...
  toolbar =  '<table cellpadding="0" cellspacing="0" border="0" class="toolbar1" style="width:' + toolbarWidth + 'px;"><tr>';
	...
	toolbar += '<td style="width: 22px;"><span id="FontSizes'  + n + '"></span></td>';
      ...
      var buttonImage          = buttonObj[2];
		  var buttonImageRollover  = buttonObj[3];
		    ...
		    toolbar += '<td style="width: 22px;">';
				toolbar += '<span id="HTMLMode' + n + '"><img src="'  +buttonImage+  '" border=0 unselectable="on" title="' +buttonTitle+ '" id="' +buttonID+ '" class="button" onClick="formatText(this.id,\'' + n + '\');" unselectable="on"  width="20" height="20"></span>';
				toolbar += '<span id="textMode' + n + '"><img src="' +imagesDir+ 'button.view_text.gif" border=0 unselectable="on" title="viewText"          id="ViewText"       class="button" onClick="formatText(this.id,\'' + n + '\');" unselectable="on"  width="20" height="20"></span>';
	      toolbar += '</td>';
			} else {
		    toolbar += '<td style="width: 22px;"><img src="' +buttonImage+ '" border=0 unselectable="on" title="' +buttonTitle+ '" id="' +buttonID+ '" class="button" onClick="formatText(this.id,\'' + n + '\');"></td>';
	    }
    }
    i++;
  }
  ...
  document.getElementById("wysiwyg" + n).contentWindow.focus();
	...
	if (viewTextMode == 1 && disabled_id == 1) {
      ...
      var currentColor = _dec_to_rgb(document.getElementById("wysiwyg" + n).contentWindow.document.queryCommandValue(id));
	    ...
	    window.open(popupsDir + 'create_table.html?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=0,resizable=0,width=350,height=170');
	  }
		...
		// iCode
		  ...
		  //document.getElementById("wysiwyg" + n).contentWindow.document.execCommand('Value', false, '[code][/code]');
...
function outputFontSizes(n) {
  ...
  var FontSizeOn         = FontSizeObj[3];
	...
	FontSizesDropDown[n] = '<table border="0" cellpadding="0" cellspacing="0"><tr><td><img src="' + FontSize + '" id="selectSize' + n + '" width="21" height="20" onClick="showFontSizes(\'' + n + '\');" unselectable="on"><br>';
  FontSizesDropDown[n] += '<span id="Sizes' + n + '" class="dropdown" style="width: 170px;">';

	for (var i = 0; i <= FontSizes.length;) {
	  if (FontSizes[i]) {
      FontSizesDropDown[n] += '<button type="button" onClick="formatText(\'FontSize\',\'' + n + '\',\'' + FontSizes[i] + '\')\;hideFontSizes(\'' + n + '\');" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut"><table cellpadding="0" cellspacing="0" border="0"><tr><td align="left" style="font-family: arial, verdana, helvetica;"><font style="font-size:' + (FontSizes[i]+1) + 'px">size ' + FontSizes[i] + '</font></td></tr></table></button><br>';	
    }	  
	  i++;
  }
