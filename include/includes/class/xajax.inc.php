...
function isClass($sClass){return is_a($this->obj,$sClass);}
...
return false;}
function generateClientScript(){if(false===$this->bDeferScriptGeneration||'deferred'===$this->bDeferScriptGeneration){if(0 < count($this->aCallableObjects)){$sCrLf="\n";print $sCrLf;print '<';print 'script type="text/javascript" ';print $this->sDefer;print 'charset="UTF-8">';print $sCrLf;print '/* <';print '![CDATA[ */';print $sCrLf;foreach(array_keys($this->aCallableObjects)as $sKey)
$this->aCallableObjects[$sKey]->generateClientScript($this->sXajaxPrefix);print '/* ]]> */';print $sCrLf;print '<';print '/script>';print $sCrLf;}
}
}
...
$this->bDeferScriptGeneration=$mValue;else if('deferred'==$mValue)
$this->bDeferScriptGeneration=$mValue;}else if('language'==$sName){$this->sLanguage=$mValue;}
}
...
function printJavascriptConfig(){$sCrLf="\n";print $sCrLf;print '<';print 'script type="text/javascript" ';print $this->sDefer;print 'charset="UTF-8">';print $sCrLf;print '/* <';print '![CDATA[ */';print $sCrLf;print 'try { if (undefined == xajax.config) xajax.config = {}; } catch (e) { xajax = {}; xajax.config = {}; };';print $sCrLf;print 'xajax.config.requestURI = "';print $this->sRequestURI;print '";';print $sCrLf;print 'xajax.config.statusMessages = ';print $this->sStatusMessages;print ';';print $sCrLf;print 'xajax.config.waitCursor = ';print $this->sWaitCursor;print ';';print $sCrLf;print 'xajax.config.version = "';print $this->sVersion;print '";';print $sCrLf;print 'xajax.config.legacy = false;';print $sCrLf;print 'xajax.config.defaultMode = "';print $this->sDefaultMode;print '";';print $sCrLf;print 'xajax.config.defaultMethod = "';print $this->sDefaultMethod;print '";';print $sCrLf;print '/* ]]> */';print $sCrLf;print '<';print '/script>';print $sCrLf;}
...
$sJsURI .='/';$sCrLf="\n";foreach($aJsFiles as $aJsFile){print '<';print 'script type="text/javascript" src="';print $sJsURI;print $aJsFile[0];print '" ';print $this->sDefer;print 'charset="UTF-8"><';print '/script>';print $sCrLf;}
if(0 < $this->nScriptLoadTimeout){foreach($aJsFiles as $aJsFile){print '<';print 'script type="text/javascript" ';print $this->sDefer;print 'charset="UTF-8">';print $sCrLf;print '/* <';print '![CDATA[ */';print $sCrLf;print 'window.setTimeout(';print $sCrLf;print ' function() {';print $sCrLf;print '  var scriptExists = false;';print $sCrLf;print '  try { if (';print $aJsFile[1];print '.isLoaded) scriptExists = true; }';print $sCrLf;print '  catch (e) {}';print $sCrLf;print '  if (!scriptExists) {';print $sCrLf;print '   alert("Error: the ';print $aJsFile[1];print ' Javascript component could not be included. Perhaps the URL is incorrect?\nURL: ';print $sJsURI;print $aJsFile[0];print '");';print $sCrLf;print '  }';print $sCrLf;print ' }, ';print $this->nScriptLoadTimeout;print ');';print $sCrLf;print '/* ]]> */';print $sCrLf;print '<';print '/script>';print $sCrLf;}
}
}
...
return $obj;}
function getVersion(){return 'xajax 0.5 Beta 4';}
...
}else if(!empty($_SERVER['SERVER_NAME'])){$aURL['host']=$_SERVER['SERVER_NAME'];}else{print $this->objLanguageManager->getText('DTCTURI:01');print $this->objLanguageManager->getText('DTCTURI:02');exit();}
}
...
$sCrLf="\n";ob_start();print $GLOBALS['xajaxErrorHandlerText'];print $sCrLf;print '----';print $sCrLf;print '[';print $errTypeStr;print '] ';print $errstr;print $sCrLf;print 'Error on line ';print $errline;print ' of file ';print $errfile;$GLOBALS['xajaxErrorHandlerText']=ob_get_clean();}

if(!defined('XAJAX_METHOD_UNKNOWN'))define('XAJAX_METHOD_UNKNOWN',0);if(!defined('XAJAX_METHOD_GET'))define('XAJAX_METHOD_GET',1);if(!defined('XAJAX_METHOD_POST'))define('XAJAX_METHOD_POST',2);class xajaxArgumentManager{var $aArgs;var $bDecodeUTF8Input;var $sCharacterEncoding;var $nMethod;var $aSequence;function argumentStripSlashes(&$sArg){if(false==is_string($sArg))
return;$sArg=stripslashes($sArg);}
function argumentDecodeXML(&$sArg){if(false==is_string($sArg))
return;if(0==strlen($sArg))
return;$nStackDepth=0;$aStack=array();$aArg=array();$nCurrent=0;$nLast=0;$aExpecting=array();$nFound=0;list($aExpecting,$nFound)=$this->aSequence['start'];$nLength=strlen($sArg);$sKey='';$mValue='';while($nCurrent < $nLength){$bFound=false;foreach($aExpecting as $sExpecting=> $nExpectedLength){if($sArg[$nCurrent]==$sExpecting[0])
if($sExpecting==substr($sArg,$nCurrent,$nExpectedLength)){list($aExpecting,$nFound)=$this->aSequence[$sExpecting];switch($nFound){case 3:
$sKey='';break;case 4:
$sKey=str_replace(
array('<'.'![CDATA[',']]>'),
'',
